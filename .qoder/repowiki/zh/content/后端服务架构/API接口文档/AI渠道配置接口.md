...
- 渠道配置读写接口：支持模型列表与默认标记
- AI模型列表查询接口：按渠道返回可用模型
...
BE_Go_DB_Setup["db/setup.go<br/>SQLite初始化"]
end
...
- 渠道配置模型与请求/响应结构
  ...
  - Node.js模型：与数据库字段映射一致，包含id、name、api_url、api_key、model_list、is_default、created_at、created_by
- 认证与授权中间件
...
- 数据库初始化
  - ai_channels表定义：id、name、api_url、api_key、model_list(JSON)、is_default、created_at、created_by
- 前端代理API
  - fetch：读取链接内容
  - image：图片生成
...
## 架构总览
后端通过统一的AI内容代理将前端请求转发至AI服务（默认目标为localhost:3000）。代理层支持多种认证方式（Header/Cookie/URL Token），并在请求头中注入用户信息，便于下游服务识别来源。
...
BE->>AI : "转发请求携带X-CMS-User/X-CMS-Role等头部"
AI-->>BE : "返回内容/图片等结果"
BE-->>FE : "封装响应统一JSON"
```
...
- 请求与响应
  - 请求体字段：name、api_url、api_key、model_list（数组）
  - 响应体字段：id、message等
- 权限控制
  - requireSuperAdmin中间件保护写操作
  - 审计日志记录关键管理动作
- 数据库
  - ai_channels表：包含name、api_url、api_key、model_list(JSON)、is_default、created_at、created_by

```mermaid
flowchart TD
Start(["请求进入"]) --> Method{"HTTP方法"}
Method --> |GET| List["查询渠道列表<br/>解析model_list JSON"]
Method --> |POST| Create["校验必填字段<br/>写入数据库"]
Method --> |PUT| Update["校验请求体<br/>更新数据库"]
...
- 接口概览
  ...
  - DELETE /api/ai-channels/:id：删除渠道（仅超级管理员）
- 请求与响应
  - 使用Gin绑定模型：CreateAIChannelRequest、UpdateAIChannelRequest
  - 返回标准JSON：id或message
- 权限控制
  - RequireSuperAdmin中间件保护写操作
  - 审计日志记录关键管理动作
- 数据库
  - ai_channels表：与Node.js版本一致
...
MW-->>Gin : "通过"
Gin->>DB : "INSERT INTO ai_channels"
DB-->>Gin : "返回新ID"
...
BE_Node --> |反向代理| AI["AI服务 (localhost:3000)"]
BE_Go --> |反向代理| AI
```
...
## 依赖分析
- 数据模型依赖
  - Go侧：models.AIChannel与models.AIChannelDB分别对应数据库字段与JSON序列化
  - Node.js侧：与数据库字段一一对应
- 路由与中间件依赖
  - 渠道配置路由依赖认证中间件与审计日志
  - AI内容代理依赖认证中间件与配置（JWT Secret、AIProxyURL）
- 数据库依赖
  - ai_channels表作为渠道配置的核心存储

```mermaid
graph TB
M["models.go<br/>AIChannel/AIChannelDB"] --> R1["ai_channels.go<br/>渠道配置路由"]
...
DB1["setup.go<br/>ai_channels表"] --> R1
DB2["setup.js<br/>ai_channels表"] --> R2
```
...
- 渠道配置API（Go/Gin）
  ...
  - POST /api/ai-channels
    - 权限：超级管理员
    - 请求体：CreateAIChannelRequest
    - 响应：{id}
  - PUT /api/ai-channels/:id
    - 权限：超级管理员
    - 请求体：UpdateAIChannelRequest
    - 响应：{message}
  - PUT /api/ai-channels/:id/set-default
    - 权限：超级管理员
    - 响应：{message}
